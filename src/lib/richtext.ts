...
export function processFootnotes(html: string): string {
  const footnoteRefs: { num: string; text: string }[] = [];

  // Extract footnote definitions from paragraphs
  const pTagPattern = /<p>\s*\[\^(\d+)\]:\s*([^<]+)<\/p>/g;
  let match;
  while ((match = pTagPattern.exec(html)) !== null) {
    footnoteRefs.push({ num: match[1], text: match[2].trim() });
  }

  // Remove footnote definition paragraphs from content
  let processedHtml = html.replace(/<p>\s*\[\^\d+\]:\s*[^<]+<\/p>\s*/g, "");

  // Replace footnote references with superscript links
  processedHtml = processedHtml.replace(/\[\^(\d+)\]/g, (_, num) => {
    return `<sup><a href="#footnote-${num}" id="footnote-ref-${num}" data-footnote-ref="" aria-describedby="footnote-label">${num}</a></sup>`;
  });

  // If we have footnotes, append the footnotes section
  if (footnoteRefs.length > 0) {
    processedHtml += "\n<hr>\n";
    processedHtml += '<section data-footnotes="" class="footnotes">\n';
    processedHtml += '<h2 id="footnote-label" class="sr-only">Footnotes</h2>\n';
    processedHtml += "<ol>\n";

    for (const footnote of footnoteRefs) {
      processedHtml += `<li id="footnote-${footnote.num}">\n`;
      processedHtml += `<p>${footnote.text} <a href="#footnote-ref-${footnote.num}" data-footnote-backref="" aria-label="Back to content" class="data-footnote-backref">↩</a></p>\n`;
      processedHtml += "</li>\n";
    }
