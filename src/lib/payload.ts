...
export function getAllPostSlugs() {
  return apiFetch<PaginatedResponse<Pick<Post, "id" | "slug">>>(
    "/api/posts?select[slug]=true&limit=1000&depth=0",
  );
}

export function getAllPageSlugs() {
  return apiFetch<PaginatedResponse<Pick<Page, "id" | "slug">>>(
    "/api/pages?select[slug]=true&limit=1000&depth=0",
  );
}
